...
def preprocessed_sentences_sql(query='''SELECT * FROM sentences;'''):
    ...
    try:
        ...
        nlp_sentnences = ard.find_re(nlp_sentences,
                                     find_val='dms_regex',
                                     search_col='words_as_string',
                                     new_col_name='dms_regex')
        nlp_sentnences = ard.find_re(nlp_sentences,
                                     find_val='dd_regex',
                                     search_col='words_as_string',
                                     new_col_name='dd_regex')
        nlp_sentnences = ard.find_re(nlp_sentences,
                                     find_val='digits_regex',
                                     search_col='words_as_string',
                                     new_col_name='digits_regex')
...
def preprocessed_sentences_csv(path):
    """
    ...
    nlp_sentences: pd.DataFrame
        pd.DataFrame with all NLP Sentences database information
    """
    header_list = ["_gddid", "sentid", "wordidx", "words",
                   "part_of_speech", "special_class",
    ...
    # Add REGEX columns to data.
    nlp_sentences = ard.convert_words_to_string(nlp_sentences,
                                                col_to_convert='words',
                                                new_col_name='words_as_string')
    nlp_sentences = ard.add_dms_re(nlp_sentences,
                                   search_col='words_as_string',
                                   new_col_name='dms_regex')
    nlp_sentences = ard.add_dd_re(nlp_sentences,
                                  search_col='words_as_string',
                                  new_col_name='dd_regex')
    nlp_sentences = ard\
        .find_digits_from_words(nlp_sentences,
                                search_col='words_as_string',
                                new_col_name='digits_from_words')
    nlp_sentences['words_l'] = nlp_sentences['words']\
        .astype(str).str.lower().transform(ast.literal_eval)
    return nlp_sentences
