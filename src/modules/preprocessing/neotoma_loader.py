...
def format_coords(df, col_to_convert, col_new_name):
    ...
    -------
    df: pd.DataFrame
        pd.DataFrame with new column `col_new_name` which contains a
        list of unique positive integer coordinates.
    """
...
def format_sitenames(df, col_to_convert, col_new_name):
    """
    Takes col_to_convert and creates a list of a set of strings that
    contain unique sitenames in lowercase.
    ...
    df: pd.DataFrame
        pd.DataFrame with new column `col_new_name` which contains a
        list of unique lower cased sitenames.
    """
    df[col_new_name] = df[col_to_convert]\
...
def grouped_coords_df(df, group_by_var='doi'):
    """
    Takes a dataframe and groups it by a key entry (document ID)
    summarizing sitenames and coordinates per document ID
    ...
    joined_df = df_sitenames.to_frame('sitenames')\
        .join(df_siteid.to_frame('siteid'))\
        .join(df_longeast.to_frame('longeast'))\
        .join(df_latnorth.to_frame('latnorth'))
