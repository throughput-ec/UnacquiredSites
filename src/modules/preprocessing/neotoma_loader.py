...
def format_sitenames(df, col_to_convert, col_new_name):
    """
    Takes col_to_convert and creates a list of a set of strings that contain
    unique sitenames in lowercase.
...
def grouped_coords_df(df, group_by_var='doi'):
    """
    Takes a dataframe and groups it by a key entry (document ID) summarizing
    sitenames and coordinates per document ID
